...
* ``-g`` | ``--file_ignore=`` BOOLEAN (optional) files to be ignored during comparison given as .gitignore entries seperated by ``::``. Binaries or large autogenerated files should be ignored during analysis, e.g., ``*.pdf``, ``package-lock.json`` or ``gradle-wrapper/``.
